...
func NewdbftsimService(client cl.Client, logDictionary string, localNet net.Neter) *dbftsimService {
	log.Debug()
...
func (ds *dbftsimService) BlockPersistCompleted(v interface{}) {
	log.Debug()
...
func (ds *dbftsimService) CheckExpectedView(viewNumber byte) {
	log.Debug()
...
func (ds *dbftsimService) CheckPolicy(transaction *Transaction) error {
	//TODO: CheckPolicy

	return nil
}

func (ds *dbftsimService) CheckSignatures() error {
	log.Debug()
	...
	if ds.context.GetSignaturesCount() >= ds.context.M() {
		...
		if err != nil {
			return NewDetailErr(err, ErrNoCode, "[dbftsimService] ,EncodePoint failed")
		}
		codehash, err := ToCodeHash(ep)
		if err != nil {
			return NewDetailErr(err, ErrNoCode, "[dbftsimService] ,ToCodeHash failed")
		}
		...
		for i, j := 0, 0; i < len(ds.context.BookKeepers) && j < ds.context.M(); i++ {
			if ds.context.Signatures[i] != nil {
				...
				if err != nil {
					...
					return NewDetailErr(err, ErrNoCode, "[dbftsimService], CheckSignatures AddContract failed.")
				}
				j++
			}
		}
...
func (ds *dbftsimService) CreateBookkeepingTransaction(nonce uint64) *Transaction {
	log.Debug()
...
func (ds *dbftsimService) ChangeViewReceived(payload *msg.ConsensusPayload, message *ChangeView) {
	log.Debug()
...
func (ds *dbftsimService) Halt() error {
	log.Debug()
	log.Info("dbftsim Stop")
	if ds.timer != nil {
		ds.timer.Stop()
	}
...
func (ds *dbftsimService) InitializeConsensus(viewNum byte) error {
...
func (ds *dbftsimService) LocalNodeNewInventory(v interface{}) {
	log.Debug()
...
func (ds *dbftsimService) NewConsensusPayload(payload *msg.ConsensusPayload) {
	log.Debug()
	ds.context.contextMu.Lock()
	defer ds.context.contextMu.Unlock()
...
func (ds *dbftsimService) GetUnverifiedTxs(txs []*tx.Transaction) []*tx.Transaction {
...
func (ds *dbftsimService) VerifyTxs(txs []*tx.Transaction) error {
	for _, t := range txs {
		if ok := ds.localNet.AppendTxnPool(t); !ok {
			return errors.New("[dbftsimService] VerifyTxs failed when AppendTxnPool.")
		}
	}
	return nil
}

func (ds *dbftsimService) PrepareRequestReceived(payload *msg.ConsensusPayload, message *PrepareRequest) {
	log.Debug()
...
func (ds *dbftsimService) PrepareResponseReceived(payload *msg.ConsensusPayload, message *PrepareResponse) {
	log.Debug()
...
func (ds *dbftsimService) RefreshPolicy() {
	log.Debug()
	con.DefaultPolicy.Refresh()
}

func (ds *dbftsimService) RequestChangeView() {
	log.Debug()
...
func (ds *dbftsimService) SignAndRelay(payload *msg.ConsensusPayload) {
	log.Debug()
...
func (ds *dbftsimService) Start() error {
	log.Debug()
	ds.started = true
...
func (ds *dbftsimService) Timeout() {
	log.Debug()
	ds.context.contextMu.Lock()
	defer ds.context.contextMu.Unlock()
...
func (ds *dbftsimService) timerRoutine() {
	log.Debug()
	for {
		select {
		case <-ds.timer.C:
